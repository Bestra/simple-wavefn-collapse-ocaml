...
let valid_dirs (x, y) (w, h) : Direction.t list =
  let add_to_list_if_true check item list =
    match check with true -> List.cons item list | false -> list
  in
  []
...
let nested_foldi (nested_array : 'a array array) ~init ~f =
  Array.foldi nested_array ~init ~f:(fun i_outer accum outer ->
...
let parse_example_matrix matrix =
  ...
  let weights, compatibilities =
    Array.foldi matrix ~init:(initial_weights, initial_compatibilities)
      ~f:(fun x (w, c) row ->
        Array.foldi row ~init:(w, c) ~f:(fun y (w', c') cur_tile ->
            let new_weights =
              Map.update w' cur_tile ~f:(fun tile_weight ->
                  match tile_weight with None -> 1 | Some i -> i + 1 )
            in
            let directions = valid_dirs (x, y) (matrix_width, matrix_height) in
            let empty_set = Set.empty (module Compatibility) in
            let new_compatibilities =
              List.fold directions ~init:empty_set ~f:(fun acc direction ->
